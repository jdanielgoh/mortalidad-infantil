  ...
  export default {
    ...
    props: {
      ...
      margen: {
        type: Object,
        default: function () {
          return {arriba: 20, abajo: 150, izquierda: 60, derecha: 20}
        }
      },
      alto_vis: {
        type: Number,
    ...
    methods: {
      configurandoDimensionesParaSVG() {
        ...
        this.ancho = document.querySelector(`#${this.barras_id}`).clientWidth * (121/10 ) - this.margen.derecha - this.margen.izquierda - this.ancho_leyenda_y
      ...
      configurandoDimensionesParaBarras() {
        ...
        for (let i = 0; i < this.variables.length; i++) {
          ...
          this.escalaX = d3.scaleBand()
              .domain([...this.datos].sort((a,b ) => b["cantidad"] - a["cantidad"] ).map(d => d[this.nombre_barra]))
              .range([0, this.ancho])
          ...
          this.eje_x.call(d3.axisBottom(this.escalaX))
              .attr("transform", `translate(${0}, ${this.alto})`)
					...
					this.eje_x.selectAll("text")
						.attr("transform","rotate(-50)translate(-6,-12)").style("text-anchor","end").style("dominant-baseline","middle")
